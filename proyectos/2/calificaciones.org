...
* Alejandro Pérez y Alberto Espinoza
- Documentación :: [[./PerezAlejandro-EspinozaAlberto/Proyecto-2.pdf]]
- Código :: [[./PerezAlejandro-EspinozaAlberto/monitor3b.py]]. Entregan
            también [[./PerezAlejandro-EspinozaAlberto/monitor.c]], pero
            indican que el proyecto es el primero.
- Lenguaje de desarrollo :: Python
- Interfaz :: Conversacional (texto) basada en menú
- Aspecto monitoreado ::
  Presentan un ejemplo de lanzado/esperado de hilos, información de
     características del CPU, memoria, tiempo encendido,
     interrupciones. 
- Uso de multitarea / sincronización ::
  - Un semáforo global utilizado a modo de señalización (aunque
    inútil, porque el flujo del programa es secuencial — =Menu()=
    invoca directamente a cada una de las funciones
  - Realizan un /ejemplo de multiprocesamiento/ en el que se lanzan
    hilos con distintas características, pero la única espera entre
    ellos es mediante =time.sleep()=, sin comunicación.
- Observaciones ::
  - Ufff... ¿Demoras de 20 segundos entre comandos? ¿Por qué no pedir
    "cualquier tecla para continuar" o algo así? Da una mala
...
- Documentación :: [[./ValerianoCristian-RamirezSimon/README.txt]]
- Código :: [[./ValerianoCristian-RamirezSimon/monitor.py]]
- Lenguaje de desarrollo :: Python
- Interfaz :: Terminal a pantalla completa (con secuencias de escape
              ANSI, no Curses); menú
- Aspecto monitoreado :: Memoria, procesos, disco, últimos usuarios,
     información de CPU, interrupciones
- Uso de multitarea / sincronización ::
     Manejo de hilos como mecanismo de control de flujo, invocación de
     funciones. Manejan mutex para asegurar que no haya impresión de
     dos rutinas simultáneas, aunque la lógica del sistema de todos
     modos no lo permitiría (es en realidad secuencial)
- Observaciones ::
  - Tuve que hacer varias adecuaciones, porque el programa presenta
    errores en todos los casos de uso de =readline()=. Los tres casos
    de:
    #+BEGIN_SRC python
    for l in a.readline()
         print a.readline()
    #+END_SRC
    hay que reemplazarlos por:
    #+BEGIN_SRC python
    for l in a.readlines():
         print l
    #+END_SRC
    En dos de los casos, además, estaba incorrectamente indentado (lo
    cual imposibilita la compilación). Me asalta la duda acerca de si
    pudieron ejecutar el programa con estos errores ☹
  - Me da gusto que hayan /dado correcta atribución/ al basarse en
    trabajo de terceros.
...
* Miguel Aguilar
Este proyecto fue copiado, sin modificación alguna, de uno [[https://github.com/gwolf/sistop-2018-1/blob/master/proyectos/2/SantigoLopez_MedinaMena/monitor.py][entregado
el año pasado por otros alumnos]] (y ni siquiera está copiado completo —
Lo que te delató son los archivos faltantes). Es válido buscar
documentarse en cómo un compañero resolvió el problema, pero no es
válido simplemente copiar sin siquiera intentar entender. No recibo
esta entrega.
